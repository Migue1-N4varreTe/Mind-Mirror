...
export class DecisionEngine {
  ...
  public evaluatePosition(
    ...
    const blockingPotential = this.calculateBlockingPotential(
      context,
      position,
    );
    factors.push({
    ...
    const futureOpportunity = this.calculateFutureOpportunity(
      context,
      position,
    );
    factors.push({
    ...
    const patternDisruption = this.calculatePatternDisruption(
      context,
      position,
    );
    factors.push({
    ...
    const specialCellAccess = this.calculateSpecialCellAccess(
      context,
      position,
    );
    factors.push({
  ...
  public alphaBetaSearch(
    ...
    if (maximizingPlayer) {
      ...
      for (const move of availableMoves) {
        ...
        const evaluation = this.alphaBetaSearch(
          newContext,
          depth - 1,
          alpha,
          beta,
  ...
  private calculateImmediateGain(
    context: GameContext,
    position: [number, number],
  ): number {
    const [row, col] = position;
  ...
  private calculateBlockingPotential(
    ...
    const proximityToFavorites = playerFavoritePositions.reduce(
      (total, fav) => {
    ...
    return Math.min(
      proximityToFavorites / playerFavoritePositions.length || 0,
      1,
    );
  }

  private calculateFutureOpportunity(
    context: GameContext,
    position: [number, number],
  ): number {
    // Evalúa las oportunidades futuras que crea este movimiento
    const adjacentCells = this.getAdjacentCells(position);
    const futureOpportunities = adjacentCells.filter(
      (cell) =>
        this.isCellEmpty(context.board, cell) ||
  ...
  private calculatePatternDisruption(
    context: GameContext,
    position: [number, number],
  ): number {
  ...
  private calculateCenterControl(position: [number, number]): number {
    const [row, col] = position;
    const centerRow = 3.5,
      centerCol = 3.5; // Centro del tablero 8x8
    const distance = Math.sqrt(
  ...
  private calculateSpecialCellAccess(
    context: GameContext,
    position: [number, number],
  ): number {
    const adjacentCells = this.getAdjacentCells(position);
  ...
  private generateCacheKey(
    ...
    const boardState = context.board
      .flat()
      .map((cell) => cell?.type || "empty")
      .join("");
  ...
  private calculateCounterEffectiveness(
    context: GameContext,
    move: any,
  ): number {
    return 0.2;
  }
  private applyRuleModification(
    context: GameContext,
    effect: any,
  ): GameContext {
    return context;
  }
  private isGameOver(context: GameContext): boolean {
    return false;
  }
  private evaluateGameState(context: GameContext): number {
    return 0;
  }
  ...
  private getSpecialCellValue(special: string): number {
    return 2;
  }
  private calculateComboValue(
    context: GameContext,
    position: [number, number],
  ): number {
    return 0;
  }
  private getPlayerFavoritePositions(context: GameContext): [number, number][] {
    return [];
  }
