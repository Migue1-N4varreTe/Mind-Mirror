...
export class HexagonalBoard {
  ...
  constructor(size: number = 5, hexSize: number = 30, origin: Point = { x: 0, y: 0 }) {
    this.size = size;
  ...
  private hexToPixel(hex: HexCoordinate): Point {
    ...
    const y = this.hexSize * (Math.sqrt(3)/2 * hex.q + Math.sqrt(3) * hex.r) + this.origin.y;
    return { x, y };
  }
  ...
  private pixelToHex(point: Point): HexCoordinate {
    ...
    const r = (-1/3 * relativeX + Math.sqrt(3)/3 * relativeY) / this.hexSize;
    const s = -q - r;
  ...
  private isValidCoordinate(coord: HexCoordinate): boolean {
    return Math.abs(coord.q) <= this.size && 
  ...
  private lerpHex(start: HexCoordinate, end: HexCoordinate, t: number): HexCoordinate {
  ...
  findShortestPath(start: HexCoordinate, end: HexCoordinate, obstacles: HexCoordinate[] = []): HexCoordinate[] {
    ...
    const obstacleSet = new Set(obstacles.map(coord => this.coordinateToKey(coord)));
  ...
  static coordinateEquals(coord1: HexCoordinate, coord2: HexCoordinate): boolean {
    return coord1.q === coord2.q && coord1.r === coord2.r && coord1.s === coord2.s;
  }
