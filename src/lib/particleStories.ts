...
export const PARTICLE_STORIES: Record<string, ParticleStory> = {
  REFLECTION_BLOOM: {
    ...
    narrative: 'Tus pensamientos brotan como flores de luz, expandiéndose en el cosmos de tu mente.',
    particles: [
      {
        type: 'flower',
        color: '#FFB6C1',
  ...
  WISDOM_CONSTELLATION: {
    ...
    narrative: 'Las estrellas de tu sabiduría se alinean, formando patrones de conocimiento ancestral.',
    particles: [
      {
        type: 'star',
        color: '#FFD700',
  ...
  EMOTIONAL_AURORA: {
    ...
    narrative: 'Tus emociones danzan como una aurora boreal en el cielo de tu consciencia.',
    particles: [
      {
        type: 'wave',
        color: '#7B68EE',
  ...
  QUANTUM_JOURNEY: {
    id: 'QUANTUM_JOURNEY',
    name: 'Viaje Cuántico',
    narrative: 'Tus ideas saltan entre dimensiones, explorando realidades infinitas de posibilidad.',
    particles: [
      {
        type: 'quantum',
        color: '#FF1493',
  ...
  MINDFUL_RAIN: {
    ...
    narrative: 'Gotas de mindfulness caen suavemente, nutriendo el jardín de tu presencia.',
    particles: [
      {
        type: 'glow',
        color: '#87CEEB',
...
export class ParticleStoryEngine {
  private activeStories: Map<string, { story: ParticleStory; startTime: number }> = new Map();
  ...
  playStoryByTrigger(trigger: ParticleStory['trigger'], emotion?: string): void {
    const availableStories = Object.values(PARTICLE_STORIES).filter(story => story.trigger === trigger);
