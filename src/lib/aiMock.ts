...
export async function emotionClassification(input: string) {
  ...
  const emotions = ["alegría", "tristeza", "miedo", "enojo", "calma", "confusión"];
  const colors = ["#FFD700", "#87CEEB", "#696969", "#FF6347", "#98FB98", "#DDA0DD"];
  const quotes = [
...
export async function narrative(inputArray: string[]) {
  ...
  return JSON.stringify({
    summary: "Hoy ha sido un día de reflexión profunda y crecimiento personal. Cada momento vivido te ha acercado más a tu esencia verdadera y a una comprensión más clara de tu camino.",
    main_emotion: "calma",
    metaphor: "Como un río que fluye hacia el océano, tus pensamientos encuentran su cauce natural hacia la sabiduría."
  });
}
