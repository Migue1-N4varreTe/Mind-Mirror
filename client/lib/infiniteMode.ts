...
export class InfiniteBoardEngine {
  ...
  evaluateExpansion(
    gameState: any,
    playerMoves: Array<{ position: [number, number] }>,
  ): boolean {
  ...
  private checkExpansionTriggers(
    gameState: any,
    playerMoves: Array<{ position: [number, number] }>,
  ): {
  ...
  private calculateGamePressure(gameState: any): number {
    ...
    const aiAggression =
      gameState.aiPersonality?.mentalState === "aggressive" ? 0.8 : 0.4;
  ...
  private getPressureExpansionZones(gameState: any): ExpansionZone[] {
    ...
    const directions: ExpansionZone["direction"][] = [
      "north",
      "south",
      "east",
      "west",
  ...
  private analyzeStrategicNeed(gameState: any): {
    ...
    const emptyCells = Array.from(this.cells.values()).filter(
      (cell) => cell.type === "empty",
    );
    const strategicCells = emptyCells.filter((cell) =>
      this.isStrategicPosition(cell),
    );
  ...
  private getExpansionBaseCoordinates(direction: ExpansionZone["direction"]): {
    x: number;
    y: number;
  } {
    const { minX, maxX, minY, maxY } = this.boardBounds;
    ...
    switch (direction) {
      case "north":
        return { x: Math.floor((minX + maxX) / 2), y: minY - 1 };
      case "south":
        return { x: Math.floor((minX + maxX) / 2), y: maxY + 1 };
      case "east":
        return { x: maxX + 1, y: Math.floor((minY + maxY) / 2) };
      case "west":
        return { x: minX - 1, y: Math.floor((minY + maxY) / 2) };
      case "northeast":
        return { x: maxX + 1, y: minY - 1 };
      case "northwest":
        return { x: minX - 1, y: minY - 1 };
      case "southeast":
        return { x: maxX + 1, y: maxY + 1 };
      case "southwest":
        return { x: minX - 1, y: maxY + 1 };
      default:
        return { x: 0, y: 0 };
    }
  }

  private createLinearExpansion(
  ...
  private createScatteredExpansion(
    base: { x: number; y: number },
    zone: ExpansionZone,
  ): InfiniteCell[] {
    const cells: InfiniteCell[] = [];
  ...
  private createOrganicExpansion(
    base: { x: number; y: number },
    zone: ExpansionZone,
  ): InfiniteCell[] {
    const cells: InfiniteCell[] = [];
    ...
    for (
      let generation = 0;
      generation < zone.cellCount && frontier.length > 0;
      generation++
    ) {
  ...
  private createFractalExpansion(
    base: { x: number; y: number },
    zone: ExpansionZone,
  ): InfiniteCell[] {
    const cells: InfiniteCell[] = [];
    ...
    const createFractalCell = (
      ...
      if (Math.random() < 0.7) {
        ...
        neighbors.forEach((neighbor) => {
          if (
            Math.random() < 0.4 &&
            !this.cells.has(`${neighbor.x},${neighbor.y}`)
          ) {
  ...
  private getNeighborCoordinates(coords: {
    x: number;
    y: number;
  ...
  getVisibleCells(
    ...
    return this.getAllCells().filter((cell) => {
      const distance =
        Math.abs(cell.coordinates.x - centerX) +
        Math.abs(cell.coordinates.y - centerY);
  ...
  getBoardStats(): any {
    ...
    const averageStability =
      cells.reduce((sum, c) => sum + c.stability, 0) / totalCells;
    const averageEnergy =
      cells.reduce((sum, c) => sum + c.energy, 0) / totalCells;
  ...
  stabilizeBoard(): void {
    ...
    this.getAllCells().forEach((cell) => {
      ...
      if (
        cell.generation < this.currentGeneration - 5 &&
        cell.stability < 0.2
      ) {
    ...
    if (cellsToRemove.length > 0) {
      console.log(
        `🧹 Tablero estabilizado: ${cellsToRemove.length} celdas removidas`,
      );
    }
  }
