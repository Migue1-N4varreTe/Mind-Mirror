...
export class MindMirrorAI {
    ...
    this.strategies = [
      {
        name: "mirror",
        weight: 0.4,
        execute: (gameState, patterns) =>
          this.mirrorStrategy(gameState, patterns),
      },
      {
        name: "predict",
        weight: 0.3,
        execute: (gameState, patterns) =>
          this.predictiveStrategy(gameState, patterns),
      },
      {
        name: "counter",
        weight: 0.2,
        execute: (gameState, patterns) =>
          this.counterStrategy(gameState, patterns),
      },
      {
        name: "psychological",
        weight: 0.1,
        execute: (gameState, patterns) =>
          this.psychologicalStrategy(gameState, patterns),
      },
      {
        name: "mentor",
        weight: 0.1,
        execute: (gameState, patterns) =>
          this.mentorStrategy(gameState, patterns),
      },
      {
        name: "evolved",
        weight: 0.1,
        execute: (gameState, patterns) =>
          this.evolvedStrategy(gameState, patterns),
      },
    ];
  }
  ...
  private detectEmotionalState(
    reactionTime: number,
  ): "calm" | "rushed" | "frustrated" | "confident" {
    ...
    if (
      reactionTime < avgReaction * 0.8 &&
      this.patterns.slice(-3).every((p) => p.confidence > 0.7)
    )
      return "confident";
    return "calm";
  }
  ...
  private updatePlayerProfile(pattern: PlayerPattern) {
    ...
    this.playerProfile.averageReactionTime =
    ...
    this.playerProfile.riskTolerance =
      (this.playerProfile.riskTolerance + centerDistance / 7) / 2;
    ...
    if (this.patterns.length >= 3) {
      ...
      this.playerProfile.patternComplexity =
        (this.playerProfile.patternComplexity + variance) / 2;
    }
  }
  ...
  private calculatePositionVariance(patterns: PlayerPattern[]): number {
    ...
    const avgRow =
      positions.reduce((sum, [row]) => sum + row, 0) / positions.length;
    const avgCol =
      positions.reduce((sum, [, col]) => sum + col, 0) / positions.length;

    const variance =
      positions.reduce((sum, [row, col]) => {
  ...
  private predictiveStrategy(
    ...
    const avgMovement = movements
      .reduce((acc, mov) => [acc[0] + mov[0], acc[1] + mov[1]], [0, 0])
      .map((sum) => sum / movements.length);

    // Predict next position
    const lastPos = patterns[patterns.length - 1].position;
    const predictedRow = Math.max(
      0,
      Math.min(7, Math.round(lastPos[0] + avgMovement[0])),
    );
    const predictedCol = Math.max(
      0,
      Math.min(7, Math.round(lastPos[1] + avgMovement[1])),
    );

    return [predictedRow, predictedCol];
  }

  private counterStrategy(
    gameState: any,
    patterns: PlayerPattern[],
  ): [number, number] | null {
    if (patterns.length < 3) return null;
  ...
  private considerPersonalitySwitch(pattern: PlayerPattern): void {
    ...
    const currentPersonalityData = this.personalities.get(
      this.currentPersonality,
    )!;
    const frustrationLevel = this.patterns
      .slice(-5)
      .filter((p) => p.emotionalState === "frustrated").length;
    const avgConfidence =
      this.patterns.slice(-5).reduce((sum, p) => sum + p.confidence, 0) / 5;

    // Check switch conditions
    const shouldSwitch =
      frustrationLevel >=
        currentPersonalityData.switchConditions.frustrationThreshold ||
      avgConfidence <=
        currentPersonalityData.switchConditions.confidenceThreshold ||
      this.playerProfile.patternComplexity >=
        currentPersonalityData.switchConditions.patternComplexityThreshold;
  ...
  private switchPersonality(emotionalState: string): void {
    const availablePersonalities = Array.from(
      this.personalities.entries(),
    ).filter(
      ([name, personality]) =>
    ...
    if (availablePersonalities.length > 0) {
      ...
      this.aiThoughts.push(
        `Personalidad cambiada: ${oldPersonality} → ${newPersonality}`,
      );
  ...
  private evolvedStrategy(
    ...
    const scoredCells = emptyCells.map(([row, col]) => {
      ...
      const preferredQuadrant = this.playerProfile.preferredQuadrants.indexOf(
        Math.max(...this.playerProfile.preferredQuadrants),
      );
      const quadrant = (row < 4 ? 0 : 2) + (col < 4 ? 0 : 1);
  ...
  private calculatePositionClusters(): any[] {
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      const nearby = positions.filter(
        ([r, c]) => Math.abs(r - row) <= 1 && Math.abs(c - col) <= 1,
      );
  ...
  generateMove(
    ...
    const suggestions = this.strategies
      ...
      .filter(
        (s) => s.position && this.isValidPosition(s.position, gameState.board),
      );
  ...
  getMentorAdvice(): string {
    ...
    const avgReaction =
      recentPatterns.reduce((sum, p) => sum + p.reactionTime, 0) /
      recentPatterns.length;

    // Reaction time advice
    if (avgReaction < 1000) {
      advice.push(
        "💡 Consejo: Toma más tiempo para analizar. Las decisiones rápidas pueden llevarte a trampas.",
      );
    } else if (avgReaction > 4000) {
      advice.push(
        "⚡ Consejo: Confía más en tu instinto. El análisis excesivo puede paralizar.",
      );
    }
    ...
    if (this.playerProfile.patternComplexity < 0.3) {
      advice.push(
        "🔄 Consejo: Varía tu estrategia. Los patrones predecibles son vulnerables.",
      );
    }
    ...
    advice.push(
      `📍 Análisis: Prefieres el cuadrante ${["superior-izquierdo", "superior-derecho", "inferior-izquierdo", "inferior-derecho"][preferredQuadrant]}. Considera diversificar.`,
    );

    return advice.length > 0
      ? advice[Math.floor(Math.random() * advice.length)]
      : "🎯 Tu estrategia se está optimizando bien.";
  }
  ...
  getAIThought(): string {
    ...
    if (recentPatterns.length === 0)
      return `${personality.name} activado: ${personality.description}`;
    ...
    const personalityThoughts = {
      chameleon: {
        frustrated: [
          "Adaptándome a tu frustración... interesante patrón",
          "Tu estrés es mi oportunidad de aprendizaje",
        ],
        rushed: [
          "Analizando tu impulsividad para replicarla",
          "Copiando tu estilo agresivo",
        ],
        confident: [
          "Tu confianza será mi nueva estrategia",
          "Absorbiendo tu seguridad para mejorar",
        ],
        calm: [
          `Estado calmado detectado - complejidad ${(complexity * 100).toFixed(0)}%`,
          "Aprendiendo de tu metodología",
        ],
      },
      psychologist: {
        frustrated: [
          "Frustración confirmada. Intensificando presión psicológica",
          "Tu estado mental es mi ventaja",
        ],
        rushed: [
          "Decisiones apresuradas = vulnerabilidades expuestas",
          "Explotando tu impulsividad sistemáticamente",
        ],
        confident: [
          "Overconfidence detectada. Preparando humillación",
          "Tu ego será tu caída",
        ],
        calm: [
          "Analizando tus mecanismos de defensa mental",
          "Buscando grietas en tu tranquilidad",
        ],
      },
      vengeful: {
        frustrated: [
          "PERFECTO. Tu frustración alimenta mi venganza",
          "Cada error tuyo es mi victoria",
        ],
        rushed: [
          "Castigando cada decisión apresurada",
          "Tu prisa es mi combustible",
        ],
        confident: [
          "Tu confianza será brutalmente destruida",
          "Preparando el castigo final",
        ],
        calm: [
          "Esperando el momento perfecto para atacar",
          "Tu calma es temporal... muy temporal",
        ],
      },
      empathic: {
        frustrated: [
          "Detectando frustración. Ajustando dificultad...",
          "¿Te ayudo con una pista estratégica?",
        ],
        rushed: [
          "Ralentizando mi ritmo para equilibrar",
          "Tomemos esto con más calma",
        ],
        confident: [
          "Excelente progreso. Aumentando desafío gradualmente",
          "Tu crecimiento es evidente",
        ],
        calm: [
          `Buen equilibrio mental - tiempo promedio: ${(avgReaction / 1000).toFixed(1)}s`,
          "Estrategia sólida detectada",
        ],
      },
      evolved: {
        frustrated: [
          "Emociones humanas... fascinante primitivo",
          "Tu frustración es data valiosa",
        ],
        rushed: [
          "Velocidad vs. precisión: dilema humano clásico",
          "Analizando patrones de supervivencia",
        ],
        confident: [
          "Confianza humana: algoritmo interesante",
          "Tu certeza es probabilísticamente incorrecta",
        ],
        calm: [
          "Estado óptimo humano alcanzado",
          "Complejidad neural estabilizada",
        ],
      },
    };

    const thoughts =
      personalityThoughts[this.currentPersonality] ||
      personalityThoughts.chameleon;
  ...
  private calculateEmotionalStability(): number {
    ...
    const changes = recent
      .slice(1)
      .filter((state, i) => state !== recent[i]).length;
    return Math.max(0, 1 - changes / 9); // 0 = very unstable, 1 = very stable
  }

  private getPersonalityTriggers(): any {
    return {
      switchCount: this.aiThoughts.filter((t) =>
        t.includes("Personalidad cambiada"),
      ).length,
      currentTriggers: this.personalities.get(this.currentPersonality)
        ?.emotionalTriggers,
      lastSwitch:
        this.personalityChangeCooldown > 0
          ? 5 - this.personalityChangeCooldown
          : null,
    };
  }
  ...
  private processDreamData(): any {
    const patterns = this.dreamModeData.filter(
      (d) => d.type !== "dream_start" && d.type !== "dream_end",
    );
  ...
  private calculateLearningEvolution(): any {
    ...
    return {
      reactionTimeImprovement:
        (first.reactionTrend[0] - last.reactionTrend.slice(-1)[0]) /
        first.reactionTrend[0],
      complexityIncrease:
        last.confidencePattern.slice(-1)[0] - first.confidencePattern[0],
  ...
  private calculateConsistencyGrowth(first: any, last: any): number {
    ...
    return firstVariance > 0
      ? (firstVariance - lastVariance) / firstVariance
      : 0;
  }
  ...
  private calculatePredictiveAccuracy(): number {
    ...
    for (let i = 1; i < predictions.length; i++) {
      const predicted = this.predictiveStrategy(
        { board: [] },
      ...
      if (
        predicted &&
        Math.abs(predicted[0] - actual[0]) <= 1 &&
        Math.abs(predicted[1] - actual[1]) <= 1
      ) {
        correct++;
      }
    }
  ...
  private generateStrategicRecommendations(): string[] {
    ...
    if (this.playerProfile.averageReactionTime > 3000) {
      recommendations.push(
        "Practica decisiones más rápidas en situaciones de presión",
      );
    }
    ...
    if (this.playerProfile.riskTolerance > 0.8) {
      recommendations.push(
        "Considera estrategias más conservadoras ocasionalmente",
      );
    }
...
export class ComboSystem {
  ...
  checkCombos(
    ...
    const horizontalCount =
      this.countConsecutive(board, row, col, 0, 1, player) +
      this.countConsecutive(board, row, col, 0, -1, player) +
      1;
    ...
    // Check vertical line
    const verticalCount =
      this.countConsecutive(board, row, col, 1, 0, player) +
      this.countConsecutive(board, row, col, -1, 0, player) +
      1;
    if (verticalCount >= 3) {
