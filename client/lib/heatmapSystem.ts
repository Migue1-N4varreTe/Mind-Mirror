...
export class HeatmapAnalyzer {
  ...
  private calculateCellIntensity(row: number, col: number): number {
    ...
    const directHits = this.heatmapData.filter(data => 
      data.position[0] === row && data.position[1] === col
    );
    ...
    this.heatmapData.forEach(data => {
      ...
      const distance = Math.sqrt(Math.pow(row - dataRow, 2) + Math.pow(col - dataCol, 2));
      
      if (distance > 0 && distance <= 2) {
        const influence = data.intensity * Math.exp(-distance * distance / 2) * 0.3;
    ...
    for (let row = 0; row < 6; row++) {
      for (let col = 0; col < 6; col++) {
        ...
        if (totalIntensity > threshold) {
          hotSpots.push({
            bounds: { minRow: row, maxRow: row + 2, minCol: col, maxCol: col + 2 },
  ...
  private identifyCornerPatterns(): any[] {
    const patterns = [];
    const corners = [
      { name: 'superior-izquierda', bounds: { minRow: 0, maxRow: 2, minCol: 0, maxCol: 2 } },
      { name: 'superior-derecha', bounds: { minRow: 0, maxRow: 2, minCol: 5, maxCol: 7 } },
      { name: 'inferior-izquierda', bounds: { minRow: 5, maxRow: 7, minCol: 0, maxCol: 2 } },
      { name: 'inferior-derecha', bounds: { minRow: 5, maxRow: 7, minCol: 5, maxCol: 7 } }
    ];
    ...
    corners.forEach(corner => {
      ...
      for (let row = corner.bounds.minRow; row <= corner.bounds.maxRow; row++) {
        for (let col = corner.bounds.minCol; col <= corner.bounds.maxCol; col++) {
          intensity += this.calculateCellIntensity(row, col);
          count++;
        }
      }
  ...
  getZoneAnalysis(): any {
    ...
    return {
      ...
      averageIntensity: zones.reduce((sum, z) => sum + z.intensity, 0) / zones.length || 0,
  ...
  generateInsights(): string[] {
    ...
    if (analysis.hotZones > 3) {
      insights.push("Tienes múltiples zonas preferidas - estrategia diversificada");
    ...
    if (analysis.patternZones > 0) {
      insights.push(`Detectados ${analysis.patternZones} patrones estratégicos claros`);
    }
...
export class MovementPredictor {
  ...
  private lastPredictions: Array<{ position: [number, number]; confidence: number }> = [];

  updatePredictions(gameState: any, playerPatterns: any[], aiAnalysis: any): Map<string, number> {
  ...
  private calculatePatternProbability(position: [number, number], patterns: any[]): number {
    ...
    const distance = Math.sqrt(
      Math.pow(targetRow - predictedRow, 2) + Math.pow(targetCol - predictedCol, 2)
    );
    
    return Math.exp(-distance / 2); // Closer to prediction = higher probability
  }

  private calculateStrategicValue(position: [number, number], gameState: any): number {
  ...
  private calculateEmotionalInfluence(position: [number, number], patterns: any[]): number {
  ...
  private calculateDistanceInfluence(position: [number, number], patterns: any[]): number {
    if (patterns.length === 0) return 0.5;
  ...
  getTopPredictions(count: number = 5): Array<{ position: [number, number]; confidence: number }> {
  ...
  validatePrediction(actualMove: [number, number]): number {
    const prediction = this.lastPredictions.find(p => 
      p.position[0] === actualMove[0] && p.position[1] === actualMove[1]
    );
