export interface StoryEvent {
  type: 'move' | 'combo' | 'special' | 'emotion' | 'ai_personality' | 'victory' | 'defeat' | 'milestone';
  timestamp: number;
  ...
  color: string;
  particleType: 'explosion' | 'stream' | 'spiral' | 'nova' | 'ripple' | 'text' | 'glow';
}
...
export class ParticleStoryEngine {
  ...
  private storyArc: 'introduction' | 'rising_action' | 'climax' | 'falling_action' | 'resolution' = 'introduction';
  ...
  private generateNarrative(eventData: any): string {
    ...
    switch (eventData.type) {
      ...
      case 'victory':
        templateKey = eventData.player === 'human' ? 'victory_human' : 'victory_ai';
        break;
    }

    const templates = this.narrativeTemplates.get(templateKey) || ['Un momento decisivo se despliega...'];
  ...
  private personalizeNarrative(template: string, eventData: any): string {
    ...
    if (eventData.position) {
      ...
      const zones = ['las fronteras', 'el centro', 'las esquinas', 'los flancos'];
  ...
  private getVisualStyle(eventData: any): { color: string; particleType: StoryEvent['particleType'] } {
    let color = '#ffffff';
  ...
  private updateStoryArc(): void {
    ...
    const averageIntensity = lastEvents.reduce((sum, e) => sum + e.intensity, 0) / lastEvents.length;
  ...
  private identifyClimax(): void {
    ...
    const significantEvents = this.currentStory.events.filter(e => 
      e.intensity > 0.7 || 
      e.type === 'combo' || 
      e.type === 'ai_personality' ||
      e.type === 'victory'
    ...
    if (significantEvents.length > 0) {
      this.currentStory.climax = significantEvents[significantEvents.length - 1];
    }
  }
  ...
  getCurrentNarrative(): string {
    const lastEvent = this.currentStory.events[this.currentStory.events.length - 1];
  ...
  getStoryMetrics(): any {
    ...
    const averageIntensity = events.reduce((sum, e) => sum + e.intensity, 0) / events.length;
  ...
  private getEmotionalSummary(): string {
    const emotionEvents = this.currentStory.events.filter(e => e.type === 'emotion');
    
    if (emotionEvents.length === 0) return "Las emociones permanecieron estables.";
    ...
    return emotionDescriptions[dominantEmotion] || "Las emociones danzaron en armonía.";
  }

  exportStory(): ParticleStory {
    this.currentStory.duration = Date.now() - (this.currentStory.events[0]?.timestamp || Date.now());
  ...
  startNewStory(): void {
    ...
    console.log("📖 Nueva historia iniciada. Historia anterior:", oldStory.title);
  }
  ...
  generateLiveCommentary(gameState: any): string {
    ...
    const recentIntensity = lastEvents.reduce((sum, e) => sum + e.intensity, 0) / lastEvents.length;
