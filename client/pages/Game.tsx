...
import {
  SPECIAL_CELLS,
  getRandomSpecialCell,
  getSpecialCellSpawnChance,
} from "@/lib/specialCells";
import {
  AchievementSystem,
  calculateGameEndData,
  type Achievement,
  type GameEndData,
...
export default function Game() {
  ...
  const toggleBoardMode = () => {
    ...
    setGameState((prev) => ({
      ...
      infiniteBoard:
        nextMode === "infinite" ? new InfiniteBoardEngine(8) : undefined,
    }));
  };
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (newBoard[row][col].type === "empty") {
        const specialType = getRandomSpecialCell(
          Math.random() > 0.7 ? "rare" : "common",
        );
        newBoard[row][col] = {
          type: "special",
          special: specialType,
  ...
  useEffect(() => {
    if (isThinking) {
      const interval = setInterval(() => {
        ...
        const currentPersonalityData =
          aiEngine.current.getCurrentPersonalityData();
        if (
          currentPersonalityData &&
          currentPersonalityData.name !== gameState.aiPersonality
        ) {
          setPersonalityHistory((prev) => [
            ...prev,
            `${gameState.aiPersonality} → ${currentPersonalityData.name}`,
          ]);
        }
      }, 2000);
  ...
  const playAgain = useCallback(() => {
    ...
    setGameState({
      ...
      hexBoard:
        gameState.boardMode === "hexagonal" ? new HexBoard(3) : undefined,
      infiniteBoard:
        gameState.boardMode === "infinite"
          ? new InfiniteBoardEngine(8)
          : undefined,
  ...
  const handleCellClick = useCallback(
    (row: number, col: number) => {
      ...
      if (cell.type === "special" && cell.special) {
        ...
        const effect = specialCell.effect.onActivate?.(
          newBoard,
          [row, col],
          "player",
        );
        ...
        setGameState((prev) => ({
          ...prev,
          specialCellsUsed: prev.specialCellsUsed + 1,
        }));
      ...
      // Check for combos
      const combos = comboSystem.current.checkCombos(
        newBoard,
        [row, col],
        "player",
      );
      ...
      const updatedGameState = {
        ...
        totalScore: {
          ...
          player: gameState.totalScore.player + scoreGain,
        },
  ...
  const endGame = useCallback(
    (finalGameState: any) => {
      const endData = calculateGameEndData(
        finalGameState,
        gameStartTime,
        reactionTimes,
      );
      ...
      const unlockedAchievements =
        achievementSystem.current.checkAchievements(enhancedEndData);
  ...
  const aiMove = useCallback(() => {
    ...
    setTimeout(
      () => {
        const aiPosition = aiEngine.current.generateMove(
          gameState,
        ...
        if (
          currentPersonality &&
          currentPersonality.name !== gameState.aiPersonality
        ) {
          setGameState((prev) => ({
            ...prev,
          ...
          if (gameState.storyMode) {
            storyEngine.current.addGameEvent({
              ...
              player: "ai",
              data: {
                oldPersonality: gameState.aiPersonality,
                newPersonality: currentPersonality.name,
              },
              intensity: 0.9,
            });
          }
        ...
        if (aiPosition) {
          ...
          if (cell.type === "special" && cell.special) {
            ...
            if (effect) {
              if (effect.boardChanges) {
                effect.boardChanges.forEach((change) => {
                  newBoard[change.position[0]][change.position[1]] =
                    change.newCell;
                });
              }
              if (effect.specialEffects) {
          ...
          const updatedAIGameState = {
            ...
            totalScore: {
              ...gameState.totalScore,
              ai: gameState.totalScore.ai + scoreGain,
            },
            moves: gameState.moves + 1,
  ...
  const getCellStyle = (cell: Cell, row: number, col: number) => {
    let baseClass =
      "w-12 h-12 border border-border rounded-lg transition-all duration-300 cursor-pointer relative overflow-hidden ";

    switch (cell.type) {
      case "empty":
        baseClass +=
          "bg-card/20 hover:bg-card/40 hover:border-neon-cyan/50 hover:scale-105";
        break;
      case "player":
    ...
    if (showVision && cell.type === "empty") {
      ...
      const prediction = aiEngine.current.generateMove(
        gameState,
        gameState.difficulty,
      );
  ...
  const getCellIcon = (cell: Cell) => {
    ...
    return (
      iconMap[cell.special as keyof typeof iconMap] || (
            ...
            <Card className="bg-card/30 backdrop-blur-sm border-neon-cyan/20">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {gameState.currentPlayer === "player" && (
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`font-mono ${timeLeft <= 10 ? "text-red-400" : "text-foreground"}`}
                      >
                        {timeLeft}s
                      </span>
                    </div>
              ...
              <CardContent>
                <div className="relative">
                  ...
                  <div
                    className={`grid gap-2 mb-4 ${gameState.boardMode === "hexagonal" ? "hexagonal-grid" : "grid-cols-8"}`}
                  >
                        ...
                        return (
                          <motion.div
                            ...
                            animate={{
                              ...
                              opacity: 1,
                              backgroundColor:
                                gameState.showHeatmap && heatIntensity > 0
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge
                      variant="outline"
                      className={`${phases[gameState.phase].color} border-current`}
                    >
                      {phases[gameState.phase].name}
                    </Badge>
                  </div>
                  <Progress
                    value={phases[gameState.phase].progress}
                    className="h-2"
                  />
                </div>
              </CardContent>
                  ...
                  {gameState.showHeatmap && (
                    <div>
                      ...
                      <div className="font-mono text-muted-foreground">
                        Detectadas:{" "}
                        {
                          Array.from(heatmapData.values()).filter(
                            (v) => v > 0.5,
                          ).length
                        }{" "}
                        áreas
                  ...
                  {gameState.showPredictions && (
                    <div>
                      ...
                      <div className="font-mono text-muted-foreground">
                        Confianza:{" "}
                        {Math.round(
                          (Array.from(predictions.values()).reduce(
                            (a, b) => a + b,
                            0,
                          ) /
                            predictions.size) *
                    ...
                    <>
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">
                          {Math.round(
                            reactionTimes.slice(-5).reduce((a, b) => a + b, 0) /
                              Math.min(5, reactionTimes.length),
                          )}
                          ms
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-mono ${
                            reactionTimes.slice(-1)[0] < 1500
                              ? "text-yellow-400"
                              : reactionTimes.slice(-1)[0] > 3000
                                ? "text-blue-400"
                                : "text-green-400"
                          }`}
                        >
